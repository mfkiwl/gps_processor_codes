...
def pearson(d1, d2):
    d1 = array(d1)
    d2 = array(d2)
    c = (mean(d2 * d1) - mean(d2) * mean(d1)) / (
                (mean(pow(d1, 2)) - pow(mean(d1), 2)) * (mean(pow(d2, 2)) - pow(mean(d2), 2)))
...
def cumulativebyvalue(user_mean, SD, user_, cond=2):
    ...
    for i in range(nr):
        v = lims[i]
        cumulative[i] = p_m_byv_beta(d1, d2, v)
...
rot_list_meanuser = [*range(0, 180, 10)]
...
root_directory = r"/Users/kelemensz/Documents/Research/GPS/process/24h/rework_ro/cmap"
