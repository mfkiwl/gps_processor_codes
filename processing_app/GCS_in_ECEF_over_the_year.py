...
def galactic_Sun_north_center_SD_directions(measurement_date, sec_of_day, user_longlatdist=geographic_position, prints=False):
  ...
  t = ts.utc(terrestrial_time[0], terrestrial_time[1], terrestrial_time[2], terrestrial_time[3], terrestrial_time[4], terrestrial_time[5])  
  sun = planets['sun']
  ...
  if prints:
    print(np.array(terrestrial_time).astype("int"), '    ', t.utc_strftime(format='%Y-%m-%d %H:%M:%S UTC'))#, '    ', tt_.tt_calendar())
...
def get_GCS_stars_in_time(day_path, date, star_check_per_day):
  ...
  galactic_Sun_north_center_SD_directions(date, sec_of_day, prints=True)
  try:
    pd.DataFrame(np.array(sun_pos)).to_csv(day_path+'/SUN_positions.csv', index=False)
  except:
    pass

  try:
    pd.DataFrame(np.array(northpole_pos)).to_csv(day_path+'/GNP_positions.csv', index=False)
  except:
    pass
  
  try:
    pd.DataFrame(np.array(center_pos)).to_csv(day_path+'/GC_positions.csv', index=False)
  except:
    pass

  try:
    pd.DataFrame(np.array(nunki_pos)).to_csv(day_path+'/Nunki_positions.csv', index=False)
  except:
    pass
  
  try:
    pd.DataFrame(np.array(capella_pos)).to_csv(day_path+'/Capella_positions.csv', index=False)
  except:
    pass

  try:
    pd.DataFrame(np.array(denebola_pos)).to_csv(day_path+'/Denebola_positions.csv', index=False)
  except:
    pass

  try:
    pd.DataFrame(np.array(sadalmelik_pos)).to_csv(day_path+'/Sadalmelik_positions.csv', index=False)
    print("\n Files Saved! \n")
  except:
    pass
...
destination_path = r"/Users/kelemensz/Documents/Research/GPS/stars_test/"
...
def get_stars_ECEF(root_location, year):
  ...
  for day_of_year in range(1, 366, 1):
    ...
    get_GCS_stars_in_time(path_of_day, date, 87)#87
