...
"""This script was written to determine GCS ower a long time period (comparable with the ecliptic period of the SUN),
but since Skyfield only covers dates 1899-07-28 23:59:18Z through 2053-10-08 23:58:51Z UT it is not possible using the
Skyfield library!"""
...
def format_time_auto_from_days_sec(date, sec_of_day, time_difference=time_diff):
    ...
    day = int(date[2])  # adjust acording to the first day of the curent week
...
def galactic_Sun_north_center_SD_directions(measurement_date, sec_of_day, user_longlatdist=geographic_position,
    ...
    t = ts.utc(terrestrial_time[0], terrestrial_time[1], terrestrial_time[2], terrestrial_time[3], terrestrial_time[4],
               terrestrial_time[5])
    sun = planets['sun']
    ...
    star_pos_s = pm.aer2ecef(az_s.degrees, alt_s.degrees, distance_s.m, user_longlatdist[0], user_longlatdist[1],
                             user_longlatdist[2])
    if prints:
        print(np.array(terrestrial_time).astype("int"), '    ',
              t.utc_strftime(format='%Y-%m-%d %H:%M:%S UTC'))  # , '    ', tt_.tt_calendar())
...
def get_GCS_stars_in_time(data_path, number_of_checks, number_of_years_between, starter_year):

    dates = [starter_year + number_of_years_between * i for i in range(number_of_checks)]
    sun_pos = []
    northpole_pos = []
    center_pos = []
    nunki_pos = []
    ...
    for year in tqdm(dates):  # loop over the time/epochs
        measurement_date = [year, 1, 1]
        star_positions = galactic_Sun_north_center_SD_directions(measurement_date, sec_of_day=0)  # OK, ECEF coordinates
    ...
    try:
        pd.DataFrame(np.array(sun_pos)).to_csv(data_path + '/SUN_positions.csv', index=False)
    except:
        pass

    try:
        pd.DataFrame(np.array(northpole_pos)).to_csv(data_path + '/GNP_positions.csv', index=False)
    except:
        pass

    try:
        pd.DataFrame(np.array(center_pos)).to_csv(data_path + '/GC_positions.csv', index=False)
    except:
        pass

    try:
        pd.DataFrame(np.array(nunki_pos)).to_csv(data_path + '/Nunki_positions.csv', index=False)
    except:
        pass

    try:
        pd.DataFrame(np.array(capella_pos)).to_csv(data_path + '/Capella_positions.csv', index=False)
    except:
        pass

    try:
        pd.DataFrame(np.array(denebola_pos)).to_csv(data_path + '/Denebola_positions.csv', index=False)
    except:
        pass

    try:
        pd.DataFrame(np.array(sadalmelik_pos)).to_csv(data_path + '/Sadalmelik_positions.csv', index=False)
        print("\n Files Saved! \n")
    except:
        pass
...
def get_stars_ECEF(root_location, number_of_checks, number_of_years_between):
    root_directory = create_root_dir(os.path.join(root_location, "STARS_GREENWICH"))

    get_GCS_stars_in_time(root_directory, number_of_checks, number_of_years_between, 1000)


destination_path = r"C:\SzabolcsKelemen\PhD\GPS\stars_greenwich/"

get_stars_ECEF(destination_path, 100, 100)
